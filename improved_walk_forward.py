"""
improved_walk_forward
=====================

Este script ejecuta un experimento de validación walk‑forward con
características técnicas, diferenciación fraccional (FFD) y
etiquetado de triple barrera basado en el ATR.  El objetivo es
evaluar si la combinación de estos elementos produce señales de
trading con mejores métricas de Sharpe y precisión direccional en
comparación con enfoques más simples.

Para cada valor de ``d`` en un conjunto predefinido se construyen
las features (retornos logarítmicos, volatilidad, momentum,
ATR14 y FFD) y se calcula una etiqueta de triple barrera
"dinámica" donde los niveles de take‑profit y stop‑loss se
determinan como múltiplos del ATR.  Se utilizan tres ventanas de
walk‑forward empleando ``TimeSeriesSplit`` para evaluar el modelo.

Las métricas calculadas incluyen la precisión direccional,
el ratio de Sharpe anualizado, el Sharpe deflacionado y una
aproximación de PBO (probabilidad de sobreajuste) comparando el
Sharpe in‑sample y out‑sample por fold.  Los resultados se
almacenan en ``improved_results.txt`` en la carpeta del script y
también se imprimen por pantalla.
"""
...
def fractional_diff_ffd(series: pd.Series, d: float, threshold: float = 1e-5) -> pd.Series:
    ...
    d: float
        Differencing order.
    threshold: float
        Threshold for truncating insignificant weights.

    Returns
    -------
    pd.Series
        Fractionally differentiated series with the same index as input.
    """
...
def dynamic_triple_barrier(close: pd.Series, atr: pd.Series, horizon: int, k_tp: float = 1.0, k_sl: float = 1.0) -> pd.Series:
    ...
    close: pd.Series
        Close price series.
    atr: pd.Series
        ATR series aligned with `close`.
    horizon: int
        Number of bars to look ahead.
    k_tp: float
        Multiplier for ATR to set take‑profit threshold.
    k_sl: float
        Multiplier for ATR to set stop‑loss threshold.

    Returns
    -------
    pd.Series
        Series of labels (+1, -1, 0) with NaNs for the last horizon samples.
    """
    n = len(close)
    ...
    for i in range(n - horizon):
        ...
        # find first index where threshold is hit
...
class WalkForwardConfig:
    n_splits: int = 3
    d_values: tuple[float | None, ...] = (None, 0.4, 0.5, 0.6)
    horizon: int = 10
    k_tp: float = 1.0
    k_sl: float = 1.0
    random_state: int = 42
...
def run_experiment(cfg: WalkForwardConfig, data_path: str = "btc_price_prediction/data/BTC-USD.csv"):
    df_raw = load_prices(data_path)
    results = {}
    for d in cfg.d_values:
        # compute features and labels
        feats = compute_features(df_raw, d)
        # compute ATR from features
        atr = feats["atr_14"]
        ...
        # remove neutral (0) labels to focus on directional signals
        data = data[data["label"] != 0.0]
        X = data.drop(columns=["label"]).values
        y = data["label"].values
        # convert labels to 0/1 for classifier: -1 -> 0, +1 -> 1
        ...
        for train_idx, test_idx in tscv.split(X):
            ...
            # compute strategy returns: +1 for predicted up, -1 for predicted down
            pos = np.where(y_pred == 1, 1.0, -1.0)
            # actual returns for horizon horizon: we align returns to test_idx positions
            # compute log returns series for all samples
            ...
            # approximate PBO: if train Sharpe > test Sharpe and test Sharpe < 0
            pbo = 1.0 if (sharpe_train > sharpe and sharpe < 0) else 0.0
            acc_scores.append(acc)
            sharpe_scores.append(sharpe)
            dsr_scores.append(dsr)
            pbo_scores.append(pbo)
        results[str(d)] = {
            ...
            "pbo": float(np.mean(pbo_scores)),
        }
    # save results
    out_path = os.path.join(os.path.dirname(__file__), "improved_results.txt")
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
    # print
    print("\nResultados del experimento mejorado:\n")
